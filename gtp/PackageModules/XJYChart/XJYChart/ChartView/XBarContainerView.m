...
@property(nonatomic, strong)
    NSMutableArray<CALayer*>* layerArray;  // value layer
@property(nonatomic, strong)
...
- (void)strokeChart {
  ...
  [fillRectArray enumerateObjectsUsingBlock:^(NSValue* _Nonnull obj,
    ...
    CALayer* fillRectShapeLayer =
        [self rectAnimationLayerWithBounds:fillRect
                                 fillColor:self.dataItemArray[idx].color];

      NSString *_result = [NSString stringWithFormat:@"%.6f", self.dataNumberArray[idx].doubleValue];
      NSString *resultString = _result.length > 6 ? [_result substringToIndex:_result.length - 5] : @"0.0";
      NSLog(@"++++++++++++:%@", _result);
    // 判断是否超过1万
      if (self.configuration.isWAN) {
          if ([self compareNumber:self.dataItemArray[idx].dataNumber.doubleValue second:1.0] == NSOrderedAscending) {
              NSString *_result = [NSString stringWithFormat:@"%.6f", self.dataNumberArray[idx].doubleValue * 10000.0];
              resultString = _result.length > 6 ? [_result substringToIndex:_result.length - 5] : @"0.0";
              //resultString = [NSString stringWithFormat:@"%@元", resultString];
          } else {
              resultString = [NSString stringWithFormat:@"%@万", resultString];
          }
      } else {
          //resultString = [NSString stringWithFormat:@"%@元", resultString];
      }
    ...
    XAnimationLabel* topLabel = [self
        topLabelWithRect:CGRectMake(fillRect.origin.x -25, fillRect.origin.y - 11,
                                    fillRect.size.width + 50, 12)
               fillColor:[UIColor clearColor]
                    text:resultString];
    [self addSubview:topLabel];
    [self.animationLabelArray addObject:topLabel];

//    [topLabel countFromCurrentTo:resultDN.floatValue
//                        toString:resultDN.stringValue
...
- (NSComparisonResult) compareNumber:(double) first second:(double) second {
    NSDecimalNumber *firstNum = [[NSDecimalNumber alloc] initWithDouble:first];
    NSDecimalNumber *secondNum = [[NSDecimalNumber alloc] initWithDouble:second];
...
- (CALayer*)rectAnimationLayerWithBounds:(CGRect)rect
  ...
  chartLine.lineJoin = kCALineCapRound;
  ...
  [gradientLayer setColors:@[(__bridge id)[fillColor colorWithAlphaComponent:0.2].CGColor,
                                 (__bridge id)fillColor.CGColor
                                 ]];
  ...
  [gradientSuperLayer setMask:chartLine];
    
  return gradientSuperLayer;
