...
- (void)strokePie {
  ...
  self.innerRadius = self.isAutoinnerRadius ? diameter / 6 : 0;
...
- (UILabel*)descriptionLabelForItemAtIndex:(NSUInteger)index {
  ...
  if (self.showAbsoluteValues) {
    titleValue = [NSString
        stringWithFormat:@"%.2f", currentDataItem.dataNumber.doubleValue];
  } else {
    titleValue = [NSString
        stringWithFormat:@"%.2f%%", (self.strokeEndArray[index].doubleValue -
  ...
  CGRect rect = [descriptionLabel.text boundingRectWithSize:CGSizeMake(descriptionLabel.frame.size.width, MAXFLOAT)
  ...
  descriptionLabel.frame = CGRectMake(
      ...
      descriptionLabel.frame.size.width, labelSize.height+5);
