...
//        largestFontSize--;
      ...
      obj.font = [UIFont systemFontOfSize:largestFontSize];
      obj.textColor = [UIColor lightTextColor];
      NSString *flag = self.configuration.isWAN ? @"%.0f万": @"%.0f";
        obj.text = [NSString
            stringWithFormat:flag, (idx) * (self.top - self.bottom) / (self.labelArray.count - 1) +
                                          self.bottom];
