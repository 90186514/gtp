...
/// Start Animation
- (void)countFromCurrentTo:(CGFloat)to toString:(NSString *) toString duration:(CGFloat)duration {
  [self countFrom:self.currentValue to:to toString:toString duration:duration];
}
...
- (void)setTextValue:(CGFloat)number {
    if (number < 1.0) {
       ...
       self.text = [NSString stringWithFormat:@"%ld", (NSInteger)number];
    }
}

- (void)countFrom:(CGFloat)from to:(CGFloat)to toString:(NSString *) toString duration:(CGFloat)duration {
